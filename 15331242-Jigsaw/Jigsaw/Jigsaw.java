...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
		...
		int Maxmovepath = 25000;
		isCompleted = false;
		//申请一个列表用于存放临接节点
		Vector<JigsawNode> adjectant = new Vector<JigsawNode>();
		//将第一个节点放入到列表中
		openList.add(beginJNode);
		while (!openList.isEmpty() && searchedNodesNum <= Maxmovepath) {
			currentJNode = openList.elementAt(0);
			//取出列表头的元素；
			if (currentJNode.equals(endJNode)) {
				//如果取出的节点和最终的节点相同的话，就结束了
				isCompleted = true;
			...
			searchedNodesNum++;
			pw.println("Searching.....Number of searched node: " + closeList.size() +
						"  Current state:" + this.currentJNode.toString());
			System.out.println("Searching.....Number of searched node: " + closeList.size() +
						"  Current state:" + this.currentJNode.toString());
			adjectant = this.findFollowJNodes(currentJNode);
			while (!adjectant.isEmpty()) {
				//将邻接节点全部放入到这个里面
				openList.addElement(adjectant.elementAt(0));
				adjectant.removeElementAt(0);
			}
		...
		for (int i = 1; i <= dimension * dimension; i++) {
			for (int j = 1; j <= dimension * dimension ; j++) {
				if (jNode.getNodesState()[i] != 0 && jNode.getNodesState()[i] == endJNode.getNodesState()[j]) {
					...
					int disx = x1 >= x2 ? x1 - x2 : x2 - x1;
					int disy = y1 >= y2 ? y1 - y2 : y2 - y1;
					distance += disx + disy;
					geometry += (int) Math.sqrt(disx * disx + disy * disy);
					break;
				}
			}
		}
		int finalnumber = (int) (s * 1 + distance * 2 +  geometry  * 2);
		jNode.setEstimatedValue(finalnumber);
	}
